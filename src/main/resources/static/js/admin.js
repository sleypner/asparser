    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            listRoles.forEach(function (arrRole, index) {
                optionsReal += `<option value="${arrRole}" selected="${selectedRoles.includes(arrRole) ? 'selected' : ''}">${arrRole}</option>`;
                optionsBox += `<div class="option ${selectedRoles.includes(arrRole) ? 'selected' : ''}" data-value="${arrRole}" >${arrRole}</div>`;

                if (selectedRoles.includes(arrRole)) {
...
$(document).ready(function() {
    ...
    $('#saveGeneralSettings').click(function() { saveSettings('general'); });
    $('#saveEmailSettings').click(function() { saveSettings('email'); });
    $('#saveSecuritySettings').click(function() { saveSettings('security'); });
    $('#saveMaintenanceSettings').click(function() { saveSettings('maintenance'); });
